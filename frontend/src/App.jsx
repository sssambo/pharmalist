...
const UNIT_MAP = { B: "Box", P: "Pack", R: "Roll", K: "Kit", TIN: "Tin" }
...
function App() {
  ...
  const unvalidatedByName = () => {
    ...
    rawMedicines.forEach((m) => {
      ...
      if (!grouped[key]) grouped[key] = { name: m.name, units: new Set() }
      grouped[key].units.add(m.unit)
    })
    const validSet = new Set(validNames.map((vn) => vn.name.toLowerCase()))
    return Object.values(grouped).filter((g) => !validSet.has(g.name.toLowerCase()))
  }
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [validRes, rawRes] = await Promise.all([validNamesAPI.getAll(), rawMedicinesAPI.getAll()])
  ...
  const handleSaveEditedRawMedicine = async ({ originalName, newName, units }) => {
    try {
      ...
      const response = await rawMedicinesAPI.updateAll(updatedRawMedicines)
  ...
  const handleUpdateCategories = async (id, categories) => {
    try {
      ...
      setValidNames(validNames.map((vn) => (vn.id === response.data.id ? response.data : vn)))
      setEditingName(null)
    } catch (error) {
  ...
  const handleValidateMedicine = async (medicine) => {
    try {
      const response = await rawMedicinesAPI.validateMedicine(medicine.name)
    ...
    <div className="app-container">
      <header className="app-header">
        ...
        <div className="header-buttons">
          <button className={`nav-btn ${step === "list" ? "active" : ""}`} onClick={() => setStep("list")}>
            Medicine List
          </button>
          <button className={`nav-btn ${step === "validated" ? "active" : ""}`} onClick={() => setStep("validated")}>
            Validate Names
          </button>
          <button className="unvalidated-btn" onClick={() => setStep("unvalidated")}>
            {unvalidatedByName().length} unvalidated
          </button>
          <button
            className={`nav-btn ${step === "manage_categories" ? "active" : ""}`}
          ...
          {editingValidName && (
            <EditValidNameModal
              ...
              onSave={async ({ id, name }) => {
                try {
                  const res = await validNamesAPI.update(id, { name })
                  setValidNames(validNames.map((vn) => (vn.id === res.data.id ? res.data : vn)))
                  setEditingValidName(null)
                } catch (e) {
                  console.error("Failed to update valid name:", e)
                }
              }}
